...
total_tons_plot <- salmon_mass %>% 
  ...
  guides(fill = F,
         color = F) +
  theme_default() +
  geom_point(data = d_short %>% group_by(year) %>% summarize(total = sum(y)) ,
             aes(y = total)) +
  NULL

species_tons_plot <- salmon_mass %>% 
  group_by(species, year, iter) %>% 
  summarize(metric_tons = sum(metric_tons)) %>% 
  group_by(species, year) %>% 
  summarize(low = quantile(metric_tons, probs = 0.05),
  ...
  guides(fill = F,
         color = F) +
  theme_default() +
  geom_point(data = d_short %>% group_by(species, year) %>% summarize(total = sum(y)) %>% 
               ungroup() %>%
               mutate(species = as.factor(species)) %>%
               mutate(species = fct_relevel(species, c("Pink", "Chum", "Sockeye"))) %>% 
               mutate(rank_species = case_when(species == "Chinook" ~ "e. Chinook",
                                                species == "Pink" ~ "b. Pink",
                                                species == "Chum" ~ "c. Chum",
                                                species == "Sockeye" ~ "d. Sockeye",
...
total_title <- ggdraw() + draw_label("Salmon escapement summed over all locations", fontface='bold')
total_plot <- plot_grid(total_tons_plot, species_tons_plot, ncol = 2,
          rel_widths = c(1, 3))
total_title_plot <- plot_grid(total_title, total_plot, ncol = 1, rel_heights = c(0.2, 1))

saveRDS(total_title_plot, file = "plots/total_title_plot.rds")
